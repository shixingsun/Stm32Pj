...
int8_t gizwitsEventProcess(eventInfo_t *info, uint8_t *data, uint32_t len)
{
  ...
  for(i=0; i<info->num; i++)
  {
    switch(info->event[i])
    {
      case EVENT_LOCK_STATUS:
        currentDataPoint.valueLock_status = dataPointPtr->valueLock_status;
        GIZWITS_LOG("Evt: EVENT_LOCK_STATUS %d \n", currentDataPoint.valueLock_status);
        if(0x01 == currentDataPoint.valueLock_status)
        {
          //user handle
        }
        else
