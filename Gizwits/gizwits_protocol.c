...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagLock_status)
    {
        info->event[info->num] = EVENT_LOCK_STATUS;
        info->num++;
        dataPoints->valueLock_status = gizDecompressionValue(LOCK_STATUS_BYTEOFFSET,LOCK_STATUS_BITOFFSET,LOCK_STATUS_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueLock_status != cur->valueLock_status)
    {
        GIZWITS_LOG("valueLock_status Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizCompressValue(LOCK_STATUS_BYTEOFFSET,LOCK_STATUS_BITOFFSET,LOCK_STATUS_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLock_status);
